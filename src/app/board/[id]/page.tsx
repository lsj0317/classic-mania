            ...
            <Card className="mx-auto max-w-[900px] shadow-sm border border-gray-100">
                <CardContent className="p-8 md:p-12">
                    <div className="flex items-center justify-between mb-6">
                        <Badge variant="secondary" className="rounded-full bg-blue-50 text-gray-700 font-bold">{post.category}</Badge>
                    ...
                    <div className="flex items-center gap-3 mb-8 pb-8 border-b border-gray-100">
                        ...
                        <div>
                            <span className="text-sm font-bold text-gray-800 block">{post.authorName} {isAuthor && <span className="text-gray-500 text-xs ml-1">(작성자)</span>}</span>
                    ...
                    {post.images.length > 0 && (
                        <div className={`grid gap-4 ${post.images.length > 1 ? 'grid-cols-2' : 'grid-cols-1'}`}>
                            {post.images.map((img, i) => (
                                <img key={i} src={img} alt="첨부 이미지" className="w-full h-auto rounded-xl shadow-md hover:scale-[1.02] transition-transform duration-300" />
                            ))}
                        </div>
                    )}
                </CardContent>
                <CardFooter className="bg-gray-50/50 p-6 flex flex-wrap items-center justify-between gap-4">
                    ...
                    {isAuthor && (
                        <div className="flex items-center gap-3">
                            <Button variant="link" size="sm" className="font-bold underline text-black" onClick={() => router.push(`/board/edit/${post.id}`)}>수정하기</Button>
                            <Button variant="default" size="sm" className="bg-black text-white rounded-lg shadow-none" onClick={handleDelete}>삭제하기</Button>
                        </div>
                    )}
                </CardFooter>
            </Card>
