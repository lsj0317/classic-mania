    ...
    return (
        <div className="space-y-2">
            {performances.map((p) => (
                <button
                    key={p.id}
                    onClick={() => router.push(`/performance/${p.id}`)}
...
const PerformanceDetail = () => {
    ...
    const { getAverageRating, getReviewsByPerformance } = useReviewStore();
    ...
    const performance = id ? detailCache[id as string] : null;
    ...
    if (!performance) {
        return (
            <div className="container mx-auto px-4 py-8 max-w-screen-md text-center">
                ...
                <Button variant="ghost" className="font-bold" onClick={() => router.back()}>
                    목록으로 돌아가기
                </Button>
            </div>
        );
    ...
    return (
        <div className="container mx-auto px-4 py-8 max-w-screen-md">
            ...
            <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
                <div className="flex items-center gap-2">
                    {id && getReviewsByPerformance(id).length > 0 && (
                        <div className="flex items-center gap-1.5 text-sm">
                            <Star className="h-4 w-4 fill-yellow-400 text-yellow-400" />
                            <span className="font-bold">{getAverageRating(id).toFixed(1)}</span>
                            <span className="text-muted-foreground text-xs">
                                ({getReviewsByPerformance(id).length}개 리뷰)
                            </span>
                        </div>
                    )}
                </div>
                <ShareButtons
                    ...
                    description={`${performance.place} | ${performance.period}`}
                />
            </div>

            {/* ── JSON-LD 구조화된 데이터 ── */}
            <JsonLd
                data={createMusicEventJsonLd({
                    name: performance.title,
                    startDate: performance.startDate,
                    endDate: performance.endDate,
                    location: performance.place,
                    image: performance.poster,
                    description: performance.synopsis || `${performance.title} - ${performance.place}에서 ${performance.period}`,
                })}
            />
            ...
            <Tabs defaultValue="detail" className="w-full">
                <TabsList className="w-full grid grid-cols-5 h-12">
                    ...
                    <TabsTrigger value="review" className="text-xs sm:text-sm font-semibold">리뷰</TabsTrigger>
