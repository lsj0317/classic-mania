    ...
    return (
        <Card className="p-4 border shadow-none">
            <div className="flex items-center justify-between mb-3">
                ...
                {!searched && (
                    <Button variant="outline" size="sm" className="text-xs gap-1" onClick={handleSearch} disabled={loading}>
            ...
            {searched && (
                <div className="space-y-2">
                    {news.length > 0 ? news.map((item, idx) => (
                        <a key={idx} href={item.originallink || item.link} target="_blank" rel="noopener noreferrer"
...
const CastPastPerformances = ({ castName }: { castName: string }) => {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                if (!cancelled) {
                    setPerformances(all.filter(p => p.title?.includes(castName) || p.cast?.includes(castName)));
                }
            } catch {
    ...
    return (
        <div className="space-y-2">
            {performances.map((p) => (
                <button key={p.id} onClick={() => router.push(`/performance/${p.id}`)}
    ...
    if (detailLoading && !performance) {
        return (
            <div className="flex justify-center items-center min-h-screen">
                <Spinner size="md" />
                <span className="ml-3 text-muted-foreground">티켓 정보를 불러오는 중...</span>
            </div>
        );
    }
    ...
    if (!performance) {
        return (
            <div className="p-10 text-center">
                <p className="text-muted-foreground mb-4">{detailError || '공연 정보를 찾을 수 없습니다.'}</p>
                <Button variant="ghost" onClick={() => router.back()}>뒤로가기</Button>
            </div>
        );
    }

    return (
        <div className="max-w-4xl mx-auto p-4 lg:p-10 min-h-screen">
            <Button
                variant="ghost"
            ...
            <div className="mb-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="aspect-[3/4] bg-muted border rounded-lg overflow-hidden shadow-lg">
                        {performance.poster ? (
                    ...
                    <div className="flex flex-col justify-between">
                        ...
                        <Card className="p-4 border-l-4 border-l-primary mt-4">
                            ...
                            <p className="text-lg font-bold">{performance.price || "가격 정보 없음"}</p>
                        </Card>
                    </div>
                </div>
            </div>
            ...
            <Tabs defaultValue="detail" className="w-full">
                ...
                <TabsContent value="detail" className="space-y-6 mt-6">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                            ...
                            {performance.runtime && (
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-muted-foreground">러닝타임:</span>
                                    <span className="font-medium">{performance.runtime}</span>
                                </div>
                            )}
                        </div>
                    </Card>
                    ...
                    {performance.synopsis && (
                        <Card className="p-5">
                            ...
                            <p className="text-sm text-muted-foreground leading-relaxed whitespace-pre-line">{performance.synopsis}</p>
                        </Card>
                    )}
                ...
                <TabsContent value="cast" className="mt-6">
                    {(performance.cast || performance.crew) ? (
                            ...
                            {castMembers.length > 0 && (
                                <Card className="p-5">
                                    <div className="flex gap-2 mb-4">
                                        <Button size="sm" variant={castSubTab === 'info' ? 'default' : 'outline'}
                                            className="text-xs font-semibold" onClick={() => setCastSubTab('info')}>
                                            출연진 정보
                                        </Button>
                                        <Button size="sm" variant={castSubTab === 'works' ? 'default' : 'outline'}
                                            className="text-xs font-semibold" onClick={() => setCastSubTab('works')}>
                                            작품활동
                                        </Button>
                                    </div>
                                    ...
                                    <div className="flex flex-wrap gap-2 mb-4">
                                        {castMembers.map((m) => (
                                            <button key={m.name} onClick={() => setSelectedCast(m.name)}
                                    ...
                                    {selectedCast && castSubTab === 'info' && (
                                        <CastProfileCard key={selectedCast} name={selectedCast}
                        ...
                        <div className="py-12 text-center text-muted-foreground text-sm">출연진 정보가 등록되지 않았습니다.</div>
                    )}
                </TabsContent>
                ...
                <TabsContent value="booking" className="mt-6 space-y-4">
                    ...
                    <Card className="p-5">
                        ...
                        {performance.relatedLinks && performance.relatedLinks.length > 0 ? (
                            ...
                            <div className="space-y-3">
                                <p className="text-sm text-muted-foreground text-center py-2">등록된 예매처 링크가 없습니다.</p>
                ...
                <TabsContent value="location" className="mt-6">
                    <Card className="p-5">
                        ...
                        <div className="mt-4 pt-4 border-t">
                            <a href={`https://map.naver.com/v5/search/${encodeURIComponent(performance.place)}`}
                               target="_blank" rel="noopener noreferrer">
